...
#windows only if they belong to the same transcript. It gives results in a    #
...
merge.by.transcript<- function ( MEDIPS.object ){
  
  #Merge by promoter and take the mean of the values
  result.merge <- aggregate.data.frame( MEDIPS.object[,4:12], 
  ...
  return <- result.merge[, c(11:16,2:10)] #put columns in a logical order
