...
```{r, eval=FALSE}
mr.edgeR.pc9<- list()
CS <-  MEDIPS.couplingVector(pattern = "CG", refObj = lista[["PC9_wt"]])
bam_wt <- lista[["PC9_wt"]]
bams_clones <- lista[9:12]
  
  for (i in 1:length(bams_clones)){
    
    bam_clon <- bams_clones[[i]]
    
    mr.edgeR.pc9[[i]] <-  MEDIPS.meth(MSet1 = bam_clon,
                                  MSet2 = bam_wt,
                                  CSet = CS, 
                                  p.adj = "bonferroni",
                                  diff.method = "edgeR", 
...
names(mr.edgeR.pc9) <- names(bams_clones)

#Select the significantly enriched or depleted regions with padj < 0.05
     
mr.edgeR.sig.pc9<- list()

  for (i in 1:length(mr.edgeR.pc9)){
  
    mr.edgeR.sig.pc9[[i]] <- MEDIPS.selectSig( results = mr.edgeR.pc9[[i]],
                                           p.value =0.05,
                                           adj = T,
                                           ratio = NULL,
                                           bg.counts= NULL)
...
names(mr.edgeR.sig.pc9)<- names(mr.edgeR.pc9)

save(list= c("mr.edgeR", "mr.edgeR.sig", "mr.edgeR.sig.gene", "anno.mart.gene", "bam_clon", "bam_wt", "bams_clones", "CS", "lista"), file="C:/Users/oncotraslacional/Documents/Arantxa/MBD-Seq_David/MEDIPSobjects.R")
