...
```{r, eval=FALSE}
 ...
 cpgs <- read.table(file="/RData/intersect_CpG_with_promotors_pcgenes_-2000_TSS_+500.bed",
...
```
The value of MEDIPs contains the significantly enriched or depleted methylated regions inside the CpG islands in 100 bp window. I will merge adjacent windows using the function merge.by.gene.
```{r, eval=FALSE}
...
for (i in 1:length(clones)){
  ...
  colnames(gained.cpgs[[i]])[15] <- "gene"
  colnames(lost.cpgs[[i]])[15] <- "gene"
}

gained.cpgs<- gained.cpgs[2:5]
gained.cpgs.merge <- list()
for (i in 1:length(clones)){
  
  gained.cpgs.merge[[i]] <- merge.by.gene (gained.cpgs[[i]])
}
